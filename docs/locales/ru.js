export default {
    ...
    exampleBasicMobileNote: 'У Air Datepicker есть режим, который позволяет открывать календарь как мобильное окно - в этом режиме он появляется по центру экрана в немного увеличенных размерах для облегчения выбора даты.',
    ...
    examplePositionPopperNote2: 'В следующем примере обработаем позицию с помощью библиотеки {popper}. Попробуйте проскролить контейнер с календарем',
